 ...
 * XXXX      XXXX
*/
  SetLights(x - 6, y - 5, 14, color);   //  START ROW -5
  SetLights(x - 6, y - 4, 14, color);   //  START ROW -4
  SetLights(x - 6, y - 3, 4, color);    //  START ROW -3
  SetLights(x + 4, y - 3, 4, color);
  SetLights(x - 6, y - 2, 4, color);    //  START ROW -2
  SetLights(x + 4, y - 2, 4, color);
  SetLights(x - 6, y - 1, 4, color);    //  START ROW -1
  SetLights(x + 4, y - 1, 4, color);
  SetLights(x - 6, y + 0, 14, color);   //  START ROW +0
  SetLights(x - 6, y + 1, 14, color);   //  START ROW +1
  SetLights(x - 6, y + 2, 4, color);    //  START ROW +2
  SetLights(x + 4, y + 2, 4, color);
  SetLights(x - 6, y + 3, 4, color);    //  START ROW +3
  SetLights(x + 4, y + 3, 4, color);
  SetLights(x - 6, y + 4, 4, color);    //  START ROW +4
  SetLights(x + 4, y + 4, 4, color);
  SetLights(x - 6, y + 5, 4, color);    //  START ROW +5
  SetLights(x + 4, y + 5, 4, color);
  SetLights(x - 6, y + 6, 4, color);    //  START ROW +6
  SetLights(x + 4, y + 6, 4, color);
}
...
*/
  ...
  SetLights(x - 6, y - 1, 4, color);    //  START ROW -1
  SetLights(x + 4, y - 1, 4, color);
  SetLights(x - 6, y + 0, 12, color);   //  START ROW +0
  SetLights(x - 6, y + 1, 12, color);   //  START ROW +1
  SetLights(x - 6, y + 2, 4, color);    //  START ROW +2
  SetLights(x + 4, y + 2, 4, color);
  SetLights(x - 6, y + 3, 4, color);    //  START ROW +3
  SetLights(x + 4, y + 3, 4, color);
  ...
  SetLights(x + 4, y + 4, 4, color);
  SetLights(x - 6, y + 5, 14, color);   //  START ROW +5
  SetLights(x - 6, y + 6, 14, color);   //  START ROW +6
}
...
void LetterMoveThrough_BRC(int x = -6, int y = 14, int frameLength = 70, int animationTime = 100)
{
  Serial.println("LetterMoveThrough_BRC()");
  int frame = GetFrame(frameLength, animationTime);

  DrawLetter_B(frame + x, y, 4);
  DrawLetter_R(frame + x + 16, y, 4);
  DrawLetter_C(frame + x + 32, y, 4);
}
...
void loop()
{
  ...
  switch (patternIndex)
  {
    ...
    case 15:  LetterMoveThrough_BRC();                          break;
